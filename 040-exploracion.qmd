...
3. **Mostrar el contenido del dataframe `df_aula`**:
   ...
   df_aula
   ```
   - **`df_aula`**: Muestra el contenido del dataframe `df_aula`. Al escribir simplemente el nombre del dataframe en la consola, R imprimirá las primeras filas del dataframe para que puedas visualizar los datos que has cargado.
...
3. **Crear y agrupar por intervalos**:
   ```r
   df_aula1 |>
     mutate(grupo = cut(altura_cm, breaks = seq(130, 175, by = 5), include.lowest = TRUE)) |>
     group_by(grupo)
   ```
   - **`df_aula1 |>`**: Utiliza el operador pipe (`|>`) para encadenar una serie de operaciones en el dataframe `df_aula1`.
   - **`mutate(grupo = cut(altura_cm, breaks = seq(130, 175, by = 5), include.lowest = TRUE))`**: Crea una nueva columna `grupo` que contiene intervalos de la columna `altura_cm` con un rango desde 130 hasta 175 cm, con una amplitud de clase de 5 cm. La opción `include.lowest = TRUE` asegura que el valor más bajo esté incluido en el primer intervalo.
   - **`group_by(grupo)`**: Agrupa los datos por los intervalos definidos en la columna `grupo`.

4. **Calcular la frecuencia absoluta**:
   ```r
   summarise(frec_abs = n())
   ```
   - **`summarise(frec_abs = n())`**: Crea un nuevo dataframe resumido que contiene la frecuencia absoluta (`frec_abs`) de cada grupo. La función `n()` cuenta el número de observaciones en cada grupo.

5. **Calcular la frecuencia relativa**:
   ```r
   mutate(frec_rel = frec_abs / sum(frec_abs))
   ```
   - **`mutate(frec_rel = frec_abs / sum(frec_abs))`**: Añade una nueva columna `frec_rel` al dataframe que contiene la frecuencia relativa. Esto se calcula dividiendo la frecuencia absoluta de cada grupo por la suma total de todas las frecuencias absolutas.

El resultado es un dataframe que muestra los intervalos de `altura_cm` (columna `grupo`), junto con las frecuencias absolutas (`frec_abs`) y las frecuencias relativas (`frec_rel`) para cada intervalo. Este proceso facilita el análisis de la distribución de los datos en intervalos específicos.
...
Este diagrama muestra su utilidad cuando representamos la distribución de un conjunto de valores más grande que nuestros once alumnos. Veamos su aplicación a los datos diarios de una fabricación de queso camembert a lo largo de un año.Los datos de esta fabricación están en el fichero `camembert.csv`.

La tabla de datos tiene esta estructura:

![](01-imagenes/camembert-tabla-datos.png)

Aunque no se muestran en la figura, la tabla está formada por 211 casos. Analicemos los datos con Excel. Los pasos a seguir son:
...
3. **Crear y agrupar por intervalos**:
   ...
   - **`mutate(grupo = cut(est, breaks = seq(40, 54, by = 1), include.lowest = TRUE))`**: Crea una nueva columna `grupo` que contiene intervalos de la columna `est` con un rango desde 40 hasta 54, con una amplitud de clase de 1. La opción `include.lowest = TRUE` asegura que el valor más bajo esté incluido en el primer intervalo.
   - **`group_by(grupo)`**: Agrupa los datos por los intervalos definidos en la columna `grupo`.
...
El resultado es un dataframe que muestra los intervalos de `est` (columna `grupo`), junto con las frecuencias absolutas (`frec_abs`) y las frecuencias relativas (`frec_rel`) para cada intervalo. Este proceso facilita el análisis de la distribución de los datos en intervalos específicos.
...
Este script nos permite visualizar la tabla de frecuencias.Aahora completamos el script con el código necesario para redirigir la tabla de frecuencias a `ggplot()` y hacer el diagrama de barras.
...
1. **Iniciar `ggplot` con las estéticas (aes)**:
   ...
   ```
   - **`ggplot(aes(x = grupo, y = frec_abs))`**: Inicia el objeto `ggplot` y define las estéticas (aes), que son las variables mapeadas a los ejes del gráfico.
     - **`x = grupo`**: Mapea la variable `grupo` al eje X.
...
2. **Agregar las barras del gráfico**:
   ...
   - **`geom_bar(stat = "identity", fill = "darkblue", alpha = 0.7, color = "black")`**: Añade una capa de barras al gráfico.
     - **`stat = "identity"`**: Especifica que las alturas de las barras deben ser iguales a los valores de `frec_abs` (en lugar de contarse las observaciones en cada grupo).
...
Este código te permitirá generar un diagrama de barras que muestra la frecuencia absoluta de cada intervalo en el eje X (grupos) y la frecuencia en el eje Y (frecuencia absoluta), con las barras coloreadas en azul oscuro y bordes negros.
...
3. **Iniciar `ggplot` con las estéticas (aes)**:
   ...
   ggplot(aes(x = mg, y = est)) +
   ```
   - **`ggplot(aes(x = mg, y = est))`**: Inicia un objeto `ggplot` y define las estéticas (aes). La variable `mg` se mapea al eje X y la variable `est` al eje Y.

4. **Agregar la capa de puntos**:
   ```r
   geom_point() +
   ```
   - **`geom_point()`**: Añade una capa de puntos al gráfico, mostrando cada observación en `mg` y `est`.
...
- **Segundo Fragmento**:
  ...
  df_camembert |>
    ...
    theme_bw()
  ```
  Este conjunto de instrucciones utiliza `ggplot2` para crear un gráfico de dispersión más personalizable, mapeando `mg` al eje X y `est` al eje Y, añadiendo una capa de puntos y aplicando un tema en blanco y negro para un diseño limpio y claro.
